import { setImg } from './idCheck'

import { mockStore } from '../../test-utils/test-utils';

import { SET_PASSPORT, SET_ADDRESS, SET_SELFIE } from 'src/config/constants';
...
describe('Loader actions', () => {
  ...
  test('setImg passed address section returns an action with type `SET_PASSPORT and img`', () => {
    store.dispatch(setImg('address', 'address-img'));
    expect(store.getActions()).toEqual([{ type: SET_ADDRESS, img: 'address-img' }]);
  });

  test('setImg passed selfie section returns an action with type `SET_PASSPORT and img`', () => {
